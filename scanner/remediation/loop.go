...
// RunLoop closes only registered deterministic R1 findings. Every accepted
// attempt becomes the immutable source for a fresh sibling candidate; the
// original workspace is never modified. Higher-risk work is reported only.
func RunLoop(options LoopOptions) (RemediationRun, error) {
	...
	remaining := classifyRemaining(activeRun, c, stoppedAssertion, stoppedCode)
	...
	result := RemediationRun{
		...
		Usage: usage, Candidates: candidates, FinalRun: activeRun, GateState: activeRun.TerminalState,
...
func classifyRemaining(run model.RunResult, c *catalog.Catalog, stoppedAssertion, stoppedCode string) []RemainingWork {
	...
	for _, result := range run.Results {
		...
		assertion := c.Assertions[result.AssertionID]
		code, reason := remainingReason(assertion, result, stoppedAssertion, stoppedCode)
...
func remainingReason(assertion model.Assertion, result model.AssertionResult, stoppedAssertion, stoppedCode string) (string, string) {

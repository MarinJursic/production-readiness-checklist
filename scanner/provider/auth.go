...
const authenticationMarker = ".everylast-authenticated"

var locateUserConfigDirectory = os.UserConfigDir

// AuthenticationDirectory returns Everylast's provider-specific credential root.
...
func AuthenticationDirectory(providerName string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("locate Everylast provider authentication directory: %w", err)
	}
	if root == "" {
		return "", fmt.Errorf("locate Everylast provider authentication directory: empty user configuration path")
	}
	return filepath.Join(root, "everylast", "provider-auth", providerName), nil
}
...
func MarkStoredAuthentication(providerName string) error {
	...
	if err := os.WriteFile(marker, []byte("everylast-provider-login-v1\n"), 0o600); err != nil {
...
// ClearStoredAuthentication removes Everylast's non-secret login marker after the
...
func storedAuthenticationDirectory(providerName string) (string, error) {
	...
	information, err := os.Lstat(path)
	if err != nil || !information.IsDir() || information.Mode()&os.ModeSymlink != 0 {
		return "", fmt.Errorf("no private Everylast %s login was found; run `everylast login %s`", providerName, providerName)
	}
	if runtime.GOOS != "windows" && information.Mode().Perm()&0o077 != 0 {
		return "", fmt.Errorf("Everylast %s login directory is accessible by other users", providerName)
	}
	...
	if runtime.GOOS != "windows" && marker.Mode().Perm()&0o077 != 0 {
		return "", fmt.Errorf("Everylast %s login marker is accessible by other users", providerName)
	}
	return path, nil
}

// AuthenticationOverrides binds a provider's own login/status/logout command
// to Everylast's private credential root. Normal HOME is left alone for the browser-

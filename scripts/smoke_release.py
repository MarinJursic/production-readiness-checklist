...
def smoke(release: pathlib.Path, version: str, commit: str) -> None:
    ...
    manifest_path = release / f"everylast_{version}_release-manifest.json"
    ...
    with tempfile.TemporaryDirectory(prefix="everylast-release-smoke-") as temporary:
        root = pathlib.Path(temporary)
        extracted = root / "native"
        extracted.mkdir()
        extract_archive(archive, extracted)
        binary_name = "everylast.exe" if goos == "windows" else "everylast"
        binary = extracted / f"everylast_{version}_{goos}_{goarch}" / binary_name
        ...
        launcher = npm_project / "node_modules" / "@marinjursic" / "everylast" / "bin" / "everylast.js"

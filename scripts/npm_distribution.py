...
SCOPE = "@marinjursic"
LAUNCHER_NAME = f"{SCOPE}/vuk"
PLATFORMS = {
    ("darwin", "arm64"): ("darwin-arm64", f"{SCOPE}/vuk-darwin-arm64", "darwin", "arm64"),
    ("darwin", "amd64"): ("darwin-x64", f"{SCOPE}/vuk-darwin-x64", "darwin", "x64"),
    ("linux", "arm64"): ("linux-arm64", f"{SCOPE}/vuk-linux-arm64", "linux", "arm64"),
    ("linux", "amd64"): ("linux-x64", f"{SCOPE}/vuk-linux-x64", "linux", "x64"),
    ("windows", "arm64"): ("win32-arm64", f"{SCOPE}/vuk-windows-arm64", "win32", "arm64"),
    ("windows", "amd64"): ("win32-x64", f"{SCOPE}/vuk-windows-x64", "win32", "x64"),
}
...
def build_packages(
    *,
    ...
    launcher_path = ROOT / "npm" / "prc" / "bin" / "vuk.js"
    ...
    platform_readme = (
        "# Vuk native package\n\n"
        "This internal platform package is installed automatically by `@marinjursic/vuk`. "
    ...
    for target, (key, package_name, npm_os, npm_cpu) in sorted(PLATFORMS.items()):
        ...
        binary_name = "vuk.exe" if target[0] == "windows" else "vuk"
        manifest = {
        ...
        metadata = common_metadata(package_name, version, f"Native {npm_os}/{npm_cpu} binary for Vuk")
        metadata.update(
            {
                "os": [npm_os],
                "cpu": [npm_cpu],
    ...
    launcher_metadata = common_metadata(LAUNCHER_NAME, version, "Safe launcher for Vuk")
    launcher_metadata.update(
        {
            "type": "module",
            "bin": {"vuk": "bin/vuk.js"},
            "files": ["bin/vuk.js", "platforms.json", "README.md", "LICENSE"],
    ...
    readme = launcher_readme_path.read_text(encoding="utf-8").replace("@marinjursic/vuk@VERSION", f"@marinjursic/vuk@{version}")
    launcher_entries = [
        ("LICENSE", license_data, 0o644),
        ("README.md", readme.encode("utf-8"), 0o644),
        ("bin/vuk.js", launcher, 0o755),

...
`vuk diff` compares a canonical prior run with a freshly inventoried target and
...
old target. `vuk diff` does not manufacture a new evidence envelope or relabel a
prior Pass.
...
`vuk diff` is an analysis and planning command. `vuk scan` still evaluates the

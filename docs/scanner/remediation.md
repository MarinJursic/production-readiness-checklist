...
Without a provider, `vuk fix` repeatedly applies only the registered R1 fixers.
...
`vuk fix --provider` connects the bounded loop to the read-only Codex or Claude

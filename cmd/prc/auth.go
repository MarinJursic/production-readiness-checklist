...
func runAuthentication(operation string, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
	...
	if operation == "login" {
		...
		fmt.Fprintf(stdout, "Ready. Run: everylast scan --ai %s\n", providerName)
	} else if operation == "logout" {
		if err := provider.ClearStoredAuthentication(providerName); err != nil {
			return exitError(exitInternal, err)
		}
		fmt.Fprintf(stdout, "Everylast's %s login has been cleared.\n", authenticationProviderTitle(providerName))
	}
	return nil
}
...
func printAuthenticationUsage(operation string, output io.Writer) {
	switch operation {
	case "login":
		fmt.Fprintln(output, "Usage: everylast login <codex|claude>")
		fmt.Fprintln(output, "Opens the provider's official sign-in flow in a private Everylast credential folder.")
	case "logout":
		fmt.Fprintln(output, "Usage: everylast logout <codex|claude>")
		fmt.Fprintln(output, "Removes the provider login stored for Everylast scans.")
	case "auth":
		fmt.Fprintln(output, "Usage: everylast auth [codex|claude]")
		fmt.Fprintln(output, "Shows whether Everylast can use each provider login.")
	}
}

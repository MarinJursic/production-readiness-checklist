...
class PublishNpmTests(unittest.TestCase):
    def packages(self) -> list[publish_npm.Package]:
        ...
        return [
            publish_npm.Package("@marinjursic/vuk-linux-x64", "1.2.3", root / "platform.tgz", "sha512-platform", "platform"),
            publish_npm.Package("@marinjursic/vuk", "1.2.3", root / "launcher.tgz", "sha512-launcher", "launcher"),
        ]

    def test_publish_is_platform_first_idempotent_and_verifies_registry_bytes(self) -> None:
        calls: list[list[str]] = []
        lookups = {
            "@marinjursic/vuk-linux-x64@1.2.3": [None, "sha512-platform"],
            "@marinjursic/vuk@1.2.3": ["sha512-launcher"],
        }
    ...
    def test_load_packages_binds_manifest_tarball_identity_and_launcher_last(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = output / f"vuk_{version}_release-manifest.json"
            ...
            self.assertEqual(packages[-1].name, "@marinjursic/vuk")

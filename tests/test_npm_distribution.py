...
class NpmDistributionTests(unittest.TestCase):
    def test_packages_are_reproducible_script_free_and_exactly_bound(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for target in npm_distribution.PLATFORMS:
                ...
                path = root / f"vuk-{target[0]}-{target[1]}{suffix}"
            ...
            self.assertEqual(
                set(launcher_files),
                {"package/LICENSE", "package/README.md", "package/bin/vuk.js", "package/package.json", "package/platforms.json"},
            )
            launcher_metadata = json.loads(launcher_files["package/package.json"])
            launcher = launcher_files["package/bin/vuk.js"]
            ...
            platform_name = "@marinjursic/vuk-linux-x64"
            ...
            self.assertEqual(manifest["binary_sha256"], hashlib.sha256(platform_files["package/bin/vuk"]).hexdigest())
    ...
    def test_package_filename_never_uses_scope_syntax(self) -> None:
        self.assertEqual(
            npm_distribution.package_filename("@marinjursic/vuk-linux-x64", "1.2.3"),
            "marinjursic-vuk-linux-x64-1.2.3.tgz",
        )

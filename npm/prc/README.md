...
```sh
npm install --save-dev --save-exact --ignore-scripts --no-audit --no-fund @marinjursic/prc@VERSION
npm exec --offline --no -- prc scan .
```

Add `"scan": "prc scan ."` to the project's `scripts` object when you want to
run `npm run --ignore-scripts scan`. The named scan command runs, but pre- and
post-scan hooks do not. The report path is printed at the end. See the main

...
const SOURCE = join(dirname(fileURLToPath(import.meta.url)), "..", "bin", "prc.js");
...
async function fixture({ corruptBinary = false, omitPlatform = false } = {}) {
  ...
  await cp(SOURCE, join(launcher, "bin", "prc.js"));
  await chmod(join(launcher, "bin", "prc.js"), 0o755);
  await writeFile(join(launcher, "package.json"), JSON.stringify({ name: "@marinjursic/prc", version: VERSION, type: "module" }));

  const key = `${process.platform}-${process.arch}`;
  const packageName = PLATFORM_NAMES[key];
  assert.ok(packageName, `test host ${key} is supported`);
  const binaryName = process.platform === "win32" ? "prc.exe" : "prc";
